...
WHERE s.`dept_id` = d.`deptId` and s.`roll_no` = '$rollNo'") or die(mysqli_error($con));
$row = mysqli_fetch_array($result);
$fPhoneNumber = $row[11];

if ($_GET['sem'] != 5 && $_GET['sem'] != 6) {
    $student_marks_details = mysqli_query($con, "SELECT * FROM `student_marks_details` m, `subjects` s
 WHERE m.`student_id` = '$rollNo' and s.`subjectId` = m.`subjectId` and s.`semester` = $_GET[sem]") or die(mysqli_error($con));
    ...
    WHERE a.`student_id` = '$rollNo' and s.`subjectId` = a.`subjectId`and s.`semester` = $_GET[sem]") or die(mysqli_error($con));
